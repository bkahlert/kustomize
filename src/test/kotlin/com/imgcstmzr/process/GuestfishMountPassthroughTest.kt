...
//            },
            "heredoc II" to { img: Path ->
                ...
                val result = Exec.Async.startShellScript(workingDirectory = imgDir) {
                    line("docker run --privileged --name ${img.fileName} --rm -i --volume $imgDir:$mountDir --volume $img:$mountImg $dockerImageName -x " + hereDoc(
                        listOf(
...
//                            "! mkdir $mountDir/$SHARED_DIRECTORY_NAME",
                ...
                Exec.Sync.execShellScript(workingDirectory = imgDir) {
                    line("docker exec -i ${img.fileName} bash -c " + listOf(
                        "ls",
                        "umount $mountDir/$SHARED_DIRECTORY_NAME",
                    ).joinToString("\n"))
                }.also { echo(tc.magenta("Result #1: $result\nResult #2: $it")) }
            },
