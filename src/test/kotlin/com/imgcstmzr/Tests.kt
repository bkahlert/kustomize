...
import com.imgcstmzr.DynamicTestsBuilder.ExpectationBuilder
import com.imgcstmzr.TestLabeler.callerSource
import com.imgcstmzr.TestLabeler.property
import com.imgcstmzr.TestLabeler.subject
...
inline fun <reified T> Iterable<T>.testEach(
    ...
    .run { map { subject -> DynamicContainer.dynamicContainer("for".subject(subject, containerNamePattern), DynamicTestsBuilder.build(subject, init)) } }
    ...
    companion object {
        ...
        private class CallbackCallingDynamicTestsBuilder<T>(private val subject: T, private val callback: (DynamicNode) -> Unit) : DynamicTestsBuilder<T> {
            ...
            override fun <R> with(description: String?, transform: T.() -> R): PropertyTestBuilder<R> {
                ...
                callback(DynamicContainer.dynamicContainer(description ?: "with".property(transform).subject(aspect), callerSource, nodes.asStream()))
