...
import com.bkahlert.koodies.functional.alsoIf
...
class ResolveSiblingKtTest {
    ...
    fun `should resolve sibling path`() {
        expectThat(Path.of("/a/b/c").resolveSibling { resolveSibling(fileName.serialized + "-x") }).serializedIsEqualTo("/a/b-x/c")
    }
    ...
    fun `should apply order`() = listOf(
        ...
        expectThat(Path.of("/a/b/c").resolveSibling(order) { resolveSibling(fileName.serialized + "-x") }).serializedIsEqualTo(expected)
    }
...
fun <T : Path> Assertion.Builder<T>.isSiblingOf(expected: Path, order: Int = 1) =
    assert("is sibling of order $order") { actual ->
        val actualNames = actual.map { name -> name.serialized }.toList()
        val otherNames = expected.map { name -> name.serialized }.toList()
