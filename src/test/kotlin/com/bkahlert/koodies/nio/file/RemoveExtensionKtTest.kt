...
class RemoveExtensionKtTest {

    @Test
    fun `should throw on empty extensions`() {
        expectCatching { tempPath("filename").removeExtension() }.isFailure().isA<IllegalArgumentException>()
    }
    ...
    fun `should remove multiple extensions`() = listOf(
        ...
        listOf(
            dynamicTest("$expected with extension \"test.ext\" removed should be $path") {
                expectThat(Path.of(expected).removeExtension("test.ext")).isEqualTo(Path.of(path))
            },

            dynamicTest("removing extension \"baz.ext\" from $expected should throw") {
                expectCatching { Path.of(expected).removeExtension("baz.ext") }.isFailure().isA<IllegalArgumentException>()
            },
        )
    }
}
