...
class ScientificKtTest {
    ...
    fun `should format scientifically`() = listOf(
        ...
        listOf(
            ...
            dynamicTest("Double: $value -> $expected") {
                expectThat(BigDecimal(value).toDouble().scientificFormat).isEqualTo(expected)
            },
        )
    }
    ...
    fun `should format to exact decimals`() = listOf(
        ...
        listOf(
            ...
            dynamicTest("Double: $value -> $expected") {
                expectThat(BigDecimal(value).toDouble().formatToExactDecimals(10)).isEqualTo(expected)
            },
        )
    }
    ...
    fun `should format up to decimals`() = listOf(
        ...
        listOf(
            ...
            dynamicTest("Double: $value -> $expected") {
                expectThat(BigDecimal(value).toDouble().formatUpToDecimals(7)).isEqualTo(expected)
            },
        )
    }
}
