...
class VirtCustomizeDockerAdaptable(val options: List<VirtCustomizeOption>, val customizationOptions: List<VirtCustomizeCustomizationOption>) :
    ...
    override val arguments: List<String>
        get() = options.partition { it is VirtCustomizeOption.DiskOption }.second.flatten() + customizationOptions.flatten()
    override val disks: List<Path>
        get() = options.partition { it is VirtCustomizeOption.DiskOption }.first.filterIsInstance<VirtCustomizeOption.DiskOption>().map { it.disk }
}
