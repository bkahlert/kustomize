...
object AnsiColors {
    fun CharSequence.colorize(fg: AnsiColorCode? = null, bg: ColorProvider? = null): String {
        if (fg != null && bg != null) return kotlin.runCatching { (fg on bg(ANSI.termColors))(asString()) }.getOrDefault(asString())
        if (fg != null && bg == null) return kotlin.runCatching { (fg)(asString()) }.getOrDefault(asString())
        if (fg == null && bg != null) return kotlin.runCatching { (bg(ANSI.termColors))(asString()) }.getOrDefault(asString())
        return asString()
    }
